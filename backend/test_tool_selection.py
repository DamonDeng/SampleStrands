...
def test_tool_selection():
    ...
    assert tool_names[0] == 'calculator', f"Expected calculator, got {tool_names[0]}"
    print("   ✅ PASSED")
    ...
    print(f"   Tools: {tool_names}")
    assert len(tools) == 1, f"Expected 1 tool, got {len(tools)}"
    assert 'calculator' in tool_names, f"Expected calculator in {tool_names}"
    print("   ✅ PASSED")
    ...
    for expected in expected_tools:
        assert expected in tool_names, f"Expected {expected} in {tool_names}"
    print("   ✅ PASSED")
    ...
    assert 'calculator' in tool_names, f"Expected calculator in {tool_names}"
    assert 'python_repl' in tool_names, f"Expected python_repl in {tool_names}"
    assert 'current_time' not in tool_names, f"current_time should not be in {tool_names}"
    print("   ✅ PASSED")
    ...
    print(f"   Tools: {tool_names}")
    assert len(tools) == 1, f"Expected 1 tool, got {len(tools)}"
    assert 'calculator' in tool_names, f"Expected calculator fallback in {tool_names}"
    print("   ✅ PASSED")
