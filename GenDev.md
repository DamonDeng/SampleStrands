...
- ✅ **Token Authentication**: Random tokens generated by Electron, passed to backend
- ✅ **Certificate Management**: Auto-generated certificates with proper cleanup
...
#### Security Flow
1. **Electron Startup** → Generate random token + certificates
2. **Backend Launch** → Pass token via environment variables
3. **Frontend API** → Include Bearer token in all requests
4. **HTTPS Communication** → All traffic encrypted with self-signed certs
5. **Backend Reuse** → Token persisted for service reuse scenarios
...
# - Suitable for security validation
```
